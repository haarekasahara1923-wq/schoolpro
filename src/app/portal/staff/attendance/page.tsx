...
export default function StaffAttendance() {
  ...
  const save = async () => {
    ...
    setSaving(false)
    if (d.success) setMsg('✅ Attendance saved and parents notified!')
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      <h1 style={{ fontSize: '18px', fontWeight: '800', color: 'white', margin: 0 }}>✅ Mark Attendance</h1>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
        ...
        <div>
          <label style={{ fontSize: '12px', color: '#94a3b8', marginBottom: '6px', display: 'block' }}>Select Section/Batch</label>
      ...
      {students.length > 0 && (
        <>
          ...
          <button onClick={save} disabled={saving}
            style={{ background: 'linear-gradient(135deg, #10b981, #059669)', border: 'none', borderRadius: '12px', padding: '14px', color: 'white', fontWeight: '700', fontSize: '15px', cursor: 'pointer', opacity: saving ? 0.6 : 1 }}>
